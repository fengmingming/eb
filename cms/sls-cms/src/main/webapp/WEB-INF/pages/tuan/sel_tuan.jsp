...
<body>
	<form class="form-inline" role="form" id="query_form">
		...
		<div class="form-group">
			<label>活动名称：</label>
			<input class="easyui-textbox" name="actName" style="width: 120px;height:25px;">
		</div>
		<div class="form-group">
			<label>上架状态：</label>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			actUp = function(actId){
				framework.startMask();
				$.post(framework.dynUrl+"/tuan/update.htm",{id:actId,state:1},function(d){
					if(d.success){
			...
			actDown = function(actId){
				framework.startMask();
				$.post(framework.dynUrl+"/tuan/update.htm",{id:actId,state:0},function(d){
					if(d.success){
						framework.alert("操作成功");
					}else{
						framework.alert(d.errMsg);
			...
			actDel = function(actId){
				$.messager.confirm("","删除确认",function(r){
					if(r){
						framework.startMask();
						$.post(framework.dynUrl+"/tuan/update.htm",{id:actId,isDel:127},function(d){
							if(d.success){
								framework.alert("操作成功");
							}else{
								framework.alert(d.errMsg);
			...
			var act_datagrid = $("#act_datagrid").datagrid({
		        ...
		        columns:[[{
		        	...
		        	field:"id",title:"操作",formatter:function(value,rowData,rowIndex){
		        		...
		        		if(rowData.isVerify&&rowData.state == 0){
		        			re = re + '<button onclick="actUp('+value+')">上架</button>';
		        		}
		        		if(rowData.state == 1){
		        			re = re + '<button onclick="actDown('+value+')">下架</button>';
		        		}
		        		re = re + '<button onclick="actDel('+value+')">删除</button>';
		        		return re;
		        	},width:50
		        }]],
