...
<script type="text/javascript" src="${staUrl }/js/order/collecting_order.js"></script>
</head>
<body>
	...
	<div id="u_info">
		...
		<div id="u_info_con" class="clear">
			...
			<div id="purchasing">
				...
				<ul id="purchasing_tb">
				<c:if test="${!empty dsOrderList }">
					<c:forEach var="item" items="${dsOrderList }">
					<li>
						<ul class="purchasing_tr clear">
							...
							<li class="pth_operate ptr_h ptr_p"><div class="ptr_c"><c:choose><c:when test="${item.state == 2 }"><a class="ptr_a_see" href="${rootUrl }/order/dsOrderDetail.htm?id=${item.id }">查看</a></c:when>
																		<c:when test="${item.isPaid == 1 }">
																	     	...
																	     	<a class="ptr_a_see" href="${rootUrl }/order/dsOrderDetail.htm?id=${item.id }">查看</a>
																	     </c:when>
																	     ...
																	     <c:otherwise><a class="ptr_a_see" href="${rootUrl }/order/dsOrderDetail.htm?id=${item.id }">查看</a></c:otherwise>
																			</c:choose>
								</div>
							</li>
						</ul>
				...
				<c:if test="${empty dsOrderList }">				
					<jsp:include page="../common/null_list_hint.jsp">
						<jsp:param name="content" value="代收订单" />
					</jsp:include>
				</c:if>					
				</ul>
				<!-- 自定义分页组件 -->
				<form id="collecting_order" action="${rootUrl }/order/queryDsOrderList.htm" method="get">
				...
				<jsp:include page="../component/page_nav.jsp">
					<jsp:param name="fm_id" value="collecting_order" />
