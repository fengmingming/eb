...
<script type="text/javascript" src="${staUrl }/js/order/purchasing_order.js"></script>
</head>
<body>
	...
	<div id="u_info">
		...
		<div id="u_info_con" class="clear">
			...
			<div id="purchasing">
				...
				<ul id="purchasing_tb">
				<c:if test="${!empty dgOrderList }">
					<c:forEach var="item" items="${dgOrderList }">
					<li>
						<ul class="purchasing_tr clear">
							...
							<li class="pth_operate ptr_h ptr_p"><div class="ptr_c"><c:choose><c:when test="${item.state == 2 }"><a class="ptr_a_see" href="${rootUrl }/order/dgOrderDetail.htm?id=${item.id }">查看</a></c:when>
																		 <c:when test="${item.isPaid == 1 }">
																	     	<a class="ptr_a_see" href="${rootUrl }/order/toPayment.htm?orderNum=${item.orderNum }&payPrice=${item.payPrice }">付款</a>
																	     	<a class="ptr_a_see" href="${rootUrl }/order/dgOrderDetail.htm?id=${item.id }">查看</a>
																	     </c:when>
																	     <c:when test="${item.status == 4 }">
																	     	<a class="ptr_a_see" href="${rootUrl }/order/dgOrderDetail.htm?id=${item.id }">查看</a>
																	     	<a class="ptr_a_state1 ptr_a_khqs" href="javascript:;" orderId="${item.id }" onclick="confirmReceipt('${item.id }',this)">客户签收</a>
																	     </c:when>
																	     <c:when test="${item.status == 5 }">
																	     	<a class="ptr_a_see" href="${rootUrl }/order/dgOrderDetail.htm?id=${item.id }">查看</a>
																	     	<a class="ptr_a_state2" href="javascript:;">已签收</a>
																	     </c:when>
																	     <c:otherwise><a class="ptr_a_see" href="${rootUrl }/order/dgOrderDetail.htm?id=${item.id }">查看</a></c:otherwise>
																			</c:choose>
								</div>
							</li>
						</ul>
				...
				<c:if test="${empty dgOrderList }">				
					<jsp:include page="../common/null_list_hint.jsp">
						<jsp:param name="content" value="代购订单" />
					</jsp:include>
				</c:if>					
				</ul>
				<!-- 自定义分页组件 -->
				<form id="purchasing_order" action="${rootUrl }/order/queryDgOrderList.htm" method="get">
				...
				<jsp:include page="../component/page_nav.jsp">
					<jsp:param name="fm_id" value="purchasing_order" />
