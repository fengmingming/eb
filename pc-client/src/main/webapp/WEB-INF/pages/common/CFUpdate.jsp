...
function challs_flash_update(){ //Flash 初始化函数
	...
	a.url = "http://img.365020.com:9080/imgupload/refund/uploadimg.htm?upload_type=refund&token="+document.getElementById("sid").value; 
	//设置服务器接收代码文件
...
function challs_flash_onCompleteData(a){ //获取服务器反馈信息事件
	...
	if(data.success){
		data = data.result;
		if($("#photoUrl"+i+"_h").val() != ""){
			i++;
			challs_flash_alterImg($("#photoUrl"+i));
			$("#photoUrl"+i).attr("src",_imgUrl+data);
			$("#photoUrl"+i+"_h").val(data);
		}else{
			challs_flash_alterImg($("#photoUrl"+i));
			$("#photoUrl"+i).attr("src",_imgUrl+data);
			$("#photoUrl"+i+"_h").val(data);
...
<style type="text/css">
	#CFUpd_filter { position: fixed; top: 0px; left: 0px; background: rgba(0, 0, 0, 0.5); filter: progid:DXImageTransform.Microsoft.gradient(startcolorstr=#7F000000,endcolorstr=#7F000000); height: 100%; width: 100%; z-index: 9999; display: none;}
		#object_dv { position: absolute; left: 50%; top: 50%; margin-left: -204px; margin-top: -160px;z-index: 10000;}
			#object_dv_a { position: absolute; top: 7px; right: 20px; font-size: 12px; color: #245891; font-family: '宋体';}
			#o_leaf { display: none; position: absolute; background: #dce6f1; width: 110px; height: 15px; bottom: 1px; left: 1px;}
</style>
<div id="CFUpd_filter">
	<input type="hidden" id="sid" value="${sessionid}">
	<div id="object_dv">
		<a id="object_dv_a" href="javascript:;">关闭</a>
		<div id="o_leaf"></div>
		<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=11,0,0,0" width="408" height="320" id="update" align="middle">
			...
			<param name="movie" value="${staUrl }/js/update.swf" />
		    <param name="quality" value="high" />
		    <param name="bgcolor" value="#ffffff" />
		    <param name="wmode" value="transparent">
		    <embed wmode="transparent" src="${staUrl }/js/update.swf" quality="high" bgcolor="#ffffff" width="408" height="320" name="update" align="middle" allowScriptAccess="always" allowFullScreen="false" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
		</object>
	</div>
	<div id="show"  style="margin-top:20px; display:none;  width:500px; text-align:left;"></div>
</div>