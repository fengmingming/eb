	...
	$("#order_status").change(function(){
		...
		if(status != 0){
			if($.trim(mobile) != ''){
				...
				$("#page").val(1);
				$("#purchasing_order").submit();
				//window.location.href="queryDgOrderList.htm?status="+status+"&timeType="+timeType+"&mobile="+mobile;	
			}else{
				$("input[name='timeType']").val(timeType);
				$("input[name='status']").val(status);
				$("#page").val(1);
				$("#purchasing_order").submit();
				//window.location.href="queryDgOrderList.htm?status="+status+"&timeType="+timeType;
			}							
		}else{
			if($.trim(mobile) != ''){
				$("input[name='timeType']").val(timeType);
				...
				$("#page").val(1);
				$("#purchasing_order").submit();
				//window.location.href="queryDgOrderList.htm?timeType="+timeType+"&mobile="+mobile;	
			}else{
				$("input[name='timeType']").val(timeType);
				$("input[name='status']").val("");
				$("#page").val(1);
				$("#purchasing_order").submit();
				//window.location.href="queryDgOrderList.htm?timeType="+timeType;
			}	
		}
	});	
	$("#order_time").change(function(){
		...
		if(status != 0){
			if($.trim(mobile) != ''){
				...
				$("#page").val(1);
				$("#purchasing_order").submit();
				//window.location.href="queryDgOrderList.htm?timeType="+timeType+"&status="+status+"&mobile="+mobile;	
			}else{
				$("input[name='timeType']").val(timeType);
				$("input[name='status']").val(status);
				$("#page").val(1);
				$("#purchasing_order").submit();
				//window.location.href="queryDgOrderList.htm?timeType="+timeType+"&status="+status;
			}	
		}else{
			if($.trim(mobile) != ''){
				$("input[name='timeType']").val(timeType);
...
function query_order_mobile(){
	...
	if(mobile.length == 11){
		if(status == 0){
			...
			$("#page").val(1);
			$("#purchasing_order").submit();
			//window.location.href="queryDgOrderList.htm?mobile="+mobile+"&timeType="+timeType;
		}else{
			$("input[name='timeType']").val(timeType);
			$("input[name='status']").val(status);
			$("input[name='mobile']").val(mobile);
			$("#page").val(1);
			$("#purchasing_order").submit();
			//window.location.href="queryDgOrderList.htm?mobile="+mobile+"&timeType="+timeType+"&status="+status;
		}
	}else{
		$("input[name='mobile']").val(mobile);
		$("#page").val(1);
		$("#purchasing_order").submit();
		//window.location.href="queryDgOrderList.htm?mobile="+mobile;
	}		
}